...
static LLVMValueRef get_frame_trace_ptr_ptr(CodeGen *g, ZigFn *fn, LLVMValueRef frame_ptr){
    // TODO(gwenzek): is this the best helper ?
    ZigType *ret_type = fn->type_entry->data.fn.fn_type_id.return_type;
    ZigType *frame_type = fn->frame_type->data.frame.locals_struct;
    return LLVMBuildStructGEP2(g->builder, get_llvm_type(g, frame_type), frame_ptr,
...
static LLVMValueRef gen_load_struct_field(CodeGen *g, ZigType *struct_type, LLVMValueRef struct_value, int field_index) {
    TypeStructField *struct_field = struct_type->data.structure.fields[field_index];
    assert(struct_field->gen_index != SIZE_MAX);
    LLVMValueRef field_ptr = LLVMBuildStructGEP2(g->builder, get_llvm_type(g, struct_type), struct_value, struct_field->gen_index, "");
    // TODO(gwenzek): align, volatile
    LLVMValueRef field_value = gen_load_untyped(g, get_llvm_type(g, struct_field->type_entry), field_ptr, 0, false, "");
    return field_value;
}

static ZigType* get_slice_ptr_type(CodeGen *g, ZigType *slice_type) {
    assert(slice_type->id == ZigTypeIdStruct);
    assert(slice_type->data.structure.special == StructSpecialSlice);
    TypeStructField *ptr_field = slice_type->data.structure.fields[slice_ptr_index];
    return resolve_struct_field_type(g, ptr_field);
}
...
static LLVMValueRef gen_load_slice_ptr(CodeGen *g, ZigType *slice_type, LLVMValueRef slice) {
    ...
    LLVMTypeRef pointee_type = get_llvm_type(g, get_slice_ptr_type(g, slice_type));
    // TODO: volatile
    LLVMValueRef ptr_value = gen_load_untyped(g, pointee_type, ptr_field_ptr, 0, false, "");
    return ptr_value;
}
...
static LLVMValueRef gen_load_slice_len(CodeGen *g, ZigType *slice_type, LLVMValueRef slice) {
    ...
    LLVMValueRef len_value = gen_load_untyped(g, g->builtin_types.entry_usize->llvm_type, len_field_ptr, 0, false, "");
    return len_value;
}
...
static LLVMValueRef get_add_error_return_trace_addr_fn(CodeGen *g) {
    ...
    LLVMValueRef index_field_ptr = LLVMBuildStructGEP2(g->builder, g->stack_trace_type->llvm_type, err_ret_trace_ptr, (unsigned)index_field_index, "");
    size_t addresses_field_index = g->stack_trace_type->data.structure.fields[1]->gen_index;
    LLVMValueRef addresses_field_ptr = LLVMBuildStructGEP2(g->builder, g->stack_trace_type->llvm_type, err_ret_trace_ptr, (unsigned)addresses_field_index, "");

    ZigType *slice_type = g->stack_trace_type->data.structure.fields[1]->type_entry;
    LLVMValueRef ptr_value = gen_load_slice_ptr(g, slice_type, addresses_field_ptr);
    LLVMValueRef len_value = gen_load_slice_len(g, slice_type, addresses_field_ptr);

    LLVMValueRef index_val = gen_load_struct_field(g, g->stack_trace_type, err_ret_trace_ptr, 0);
    ...
    LLVMValueRef address_slot = LLVMBuildInBoundsGEP2(g->builder, arg_types[1], ptr_value, address_indices, 1, "");
...
static void add_sentinel_check(CodeGen *g, LLVMValueRef sentinel_elem_ptr, ZigValue *sentinel) {
    ...
    LLVMValueRef actual_sentinel = gen_load_untyped(g, get_llvm_type(g, sentinel->type), sentinel_elem_ptr, 0, false, "");
    LLVMValueRef ok_bit;
        ...
        LLVMValueRef value)
{
    ...
    if (handle_is_ptr(g, child_type)) {
        ...
        ZigLLVMBuildMemCpy(g->builder, ptr, dest_align_bytes, value, src_align_bytes,
    ...
    LLVMTypeRef containing_int_ty = LLVMIntType(host_int_bytes * 8);
    LLVMValueRef containing_int = gen_load_untyped(g, containing_int_ty, ptr, get_ptr_align(g, ptr_type), ptr_type->data.pointer.is_volatile, "");
        ...
        if (abi_class == X64CABIClass_MEMORY || abi_class == X64CABIClass_MEMORY_nobyval) {
            ...
            switch (fn_walk->id) {
                ...
                case FnWalkIdCall: {
                    ...
                    LLVMValueRef loaded = LLVMBuildLoad2(g->builder, ptr_to_int_type_ref, val, "");
            ...
            switch (fn_walk->id) {
                ...
                case FnWalkIdCall: {
                    ...
                    for (uint32_t i = 0; i < number_of_regs; i += 1) {
                        ...
                        // TODO(gwenzek): we're splitting a struct in several chuncks, not sure what's the correct type here.
...
static LLVMValueRef get_merge_err_ret_traces_fn_val(CodeGen *g) {
    ...
    LLVMValueRef frame_index_ptr = LLVMBuildAlloca(g->builder, usize, "frame_index");
    LLVMValueRef frames_left_ptr = LLVMBuildAlloca(g->builder, usize, "frames_left");
    ...
    LLVMValueRef src_index_val = gen_load_struct_field(g, g->stack_trace_type, src_stack_trace_ptr, 0);

    size_t src_addresses_field_index = g->stack_trace_type->data.structure.fields[1]->gen_index;
    ZigType *slice_type = g->stack_trace_type->data.structure.fields[1]->type_entry;
    LLVMValueRef src_addresses_field_ptr = LLVMBuildStructGEP2(g->builder, get_llvm_type(g, g->stack_trace_type),
            src_stack_trace_ptr, (unsigned)src_addresses_field_index, "");
    LLVMValueRef src_ptr_val = gen_load_slice_ptr(g, slice_type, src_addresses_field_ptr);
    LLVMValueRef src_len_val = gen_load_slice_len(g, slice_type, src_addresses_field_ptr);
    ...
    LLVMValueRef usize_zero = LLVMConstNull(usize);
    ...
    LLVMValueRef ptr_index = LLVMBuildLoad2(g->builder, usize, frame_index_ptr, "");
    LLVMValueRef addr_ptr = LLVMBuildInBoundsGEP2(g->builder, usize, src_ptr_val, &ptr_index, 1, "");
    LLVMValueRef this_addr_val = LLVMBuildLoad2(g->builder, usize, addr_ptr, "");
    ...
    LLVMValueRef prev_frames_left = LLVMBuildLoad2(g->builder, usize, frames_left_ptr, "");
    ...
    LLVMValueRef prev_index = LLVMBuildLoad2(g->builder, usize, frame_index_ptr, "");
        ...
        Stage1AirInstSaveErrRetAddr *save_err_ret_addr_instruction)
{
    ...
    if (fn_is_async(g->cur_fn) && codegen_fn_has_err_ret_tracing_arg(g, ret_type)) {
        LLVMValueRef trace_ptr_ptr = get_frame_trace_ptr_ptr(g, g->cur_fn, g->cur_frame_ptr);
...
static LLVMValueRef gen_maybe_atomic_op(CodeGen *g, LLVMAtomicRMWBinOp op, LLVMTypeRef type, LLVMValueRef ptr, LLVMValueRef val,
        LLVMAtomicOrdering order)
{
    if (g->is_single_threaded) {
        LLVMValueRef loaded = LLVMBuildLoad2(g->builder, type, ptr, "");
...
static void gen_async_return(CodeGen *g, Stage1AirInstReturn *instruction) {
    ...
    LLVMValueRef prev_val = gen_maybe_atomic_op(g, LLVMAtomicRMWBinOpXor, usize_type_ref,
            g->cur_async_awaiter_ptr, all_ones, LLVMAtomicOrderingAcquire);
    ...
    if (ret_type_has_bits) {
        ...
        ZigLLVMBuildMemCpy(g->builder,
                awaiter_ret_ptr, abi_align,
                g->cur_ret_ptr, abi_align, byte_count_val, is_volatile);
...
static LLVMValueRef gen_convert_to_c_abi(CodeGen *g, LLVMValueRef location, LLVMValueRef value) {
    ...
    if (size < 8) {
        // TOOD(gwenzek): use opaque pointer so that we can remove the bitcast
...
static LLVMValueRef ir_render_return(CodeGen *g, Stage1Air *executable, Stage1AirInstReturn *instruction) {
    ...
    {
        if (instruction->operand == nullptr) {
            LLVMValueRef by_val_value = LLVMBuildLoad2(g->builder, get_llvm_type(g, fn_type_id->return_type), g->cur_ret_ptr, "");
            LLVMBuildRet(g->builder, by_val_value);
        } else {
            LLVMValueRef value = ir_llvm_value(g, instruction->operand);
            LLVMValueRef by_val_value = LLVMBuildLoad2(g->builder, get_llvm_type(g, fn_type_id->return_type), value, "");
            LLVMBuildRet(g->builder, by_val_value);
        }
        ...
        Stage1AirInstBitCast *instruction)
{
    ...
    if (wanted_is_ptr == actual_is_ptr) {
        ...
        return gen_load_untyped(g, get_llvm_type(g, wanted_type), bitcasted_ptr, alignment, false, "");
    } else {
...
static LLVMValueRef ir_render_elem_ptr(CodeGen *g, Stage1Air *executable, Stage1AirInstElemPtr *instruction) {
        ...
        (array_type->id == ZigTypeIdPointer && array_type->data.pointer.ptr_len == PtrLenSingle))
    {
        ...
        ZigType *array_content_type = get_slice_ptr_type(g, array_type);
        ...
        if (instruction->modifier == CallModifierAsync) {
            ...
            if (ir_want_runtime_safety(g, &instruction->base)) {
                LLVMValueRef awaiter_ptr = LLVMBuildStructGEP(g->builder, frame_result_loc,
                        frame_awaiter_index, "");
                LLVMValueRef all_ones = LLVMConstAllOnes(usize_type_ref);
                LLVMValueRef prev_val = gen_maybe_atomic_op(g, LLVMAtomicRMWBinOpXchg, usize_type_ref,
                        awaiter_ptr, all_ones, LLVMAtomicOrderingRelease);
    ...
    Stage1AirInstUnionFieldPtr *instruction)
{
    ...
    if (instruction->initializing) {
        ...
        LLVMValueRef tag_field_ptr = LLVMBuildStructGEP(g->builder, union_ptr, union_type->data.unionation.gen_tag_index, "");
        LLVMValueRef tag_value = gen_load_untyped(g, union_type->data.unionation.tag_type->llvm_type, tag_field_ptr, 0, false, "");
...
static LLVMValueRef gen_non_null_bit(CodeGen *g, ZigType *maybe_type, LLVMValueRef maybe_handle) {
    ...
    // return gen_load_struct_field(g, maybe_type, maybe_handle, maybe_null_index);
...
static LLVMValueRef ir_render_slice(CodeGen *g, Stage1Air *executable, Stage1AirInstSlice *instruction) {
        ...
        (array_type->id == ZigTypeIdPointer && array_type->data.pointer.ptr_len == PtrLenSingle))
    {
        ...
        if (ptr_has_bits) {
            ...
            LLVMValueRef src_ptr = gen_load_struct_field(g, array_type, array_ptr, slice_ptr_index);
            if (sentinel != nullptr) {
        ...
        Stage1AirInstAtomicLoad *instruction)
{
    ...
    if (actual_abi_type != nullptr) {
        ...
        ZigType *ptr_type = instruction->ptr->value->type;
        uint32_t alignment = get_ptr_align(g, ptr_type);
        LLVMValueRef load_inst = gen_load_untyped(g, actual_abi_type, ptr, alignment, ptr_type->data.pointer.is_volatile, "");
        ...
        Stage1AirInstArrayToVector *instruction)
{
    ...
    if (bitcast_ok) {
        ...
        assert(array_type->id == ZigTypeIdArray);
        assert(array_type->size_in_bits == vector_type->data.vector.len * elem_type->size_in_bits);
        ...
        for (uintptr_t i = 0; i < vector_type->data.vector.len; i++) {
            LLVMValueRef index_usize = LLVMConstInt(usize_type_ref, i, false);
            LLVMValueRef index_u32 = LLVMConstInt(u32_type_ref, i, false);
            LLVMValueRef indexes[] = { zero, index_usize };
            // TODO(gwenzek):  LLVMBuildInBoundsGEP2 and LLVMBuildLoad2
            LLVMValueRef elem_ptr = LLVMBuildInBoundsGEP(g->builder, array_ptr, indexes, 2, "");
        ...
        LLVMValueRef result_loc, bool non_async)
{
    ...
    if (type_has_bits(g, result_type) && (non_async || result_loc != nullptr)) {
        ...
        if (result_loc != nullptr) {
            LLVMTypeRef ptr_u8 = LLVMPointerType(LLVMInt8Type(), 0);
            // TODO(gwenzek): is bitcast necessary ?
...
static LLVMValueRef ir_render_await(CodeGen *g, Stage1Air *executable, Stage1AirInstAwait *instruction) {
    ...
    LLVMValueRef prev_val = gen_maybe_atomic_op(g, LLVMAtomicRMWBinOpXchg, usize_type_ref,
            awaiter_ptr, awaiter_init_val, LLVMAtomicOrderingRelease);
        ...
        ZigType **field)
{
    ...
    entry->size_in_bits = LLVMSizeOfTypeInBits(g->target_data_ref, entry->llvm_type);
...
static void define_builtin_types(CodeGen *g) {
    ...
    for (size_t i = 0; i < array_length(c_int_type_infos); i += 1) {
        ...
        entry->llvm_di_type = ZigLLVMCreateDebugBasicType(g->dbuilder, buf_ptr(&entry->name),
                LLVMSizeOfTypeInBits(g->target_data_ref, entry->llvm_type),
