...
// TODO(gwenzek): remove as much as possible, since this isn't really untypped anymore
...
static LLVMValueRef gen_load(CodeGen *g, LLVMValueRef ptr, ZigType *ptr_type, const char *name) {
    ...
    LLVMValueRef result = LLVMBuildLoad2(g->builder, get_llvm_type(g, elem_type), ptr, name);
    if (ptr_type->data.pointer.is_volatile) LLVMSetVolatile(result, true);
    if (alignment == 0) {
        LLVMSetAlignment(result, LLVMABIAlignmentOfType(g->target_data_ref, LLVMGetElementType(LLVMTypeOf(ptr))));
    } else {
        LLVMSetAlignment(result, alignment);
...
static LLVMTypeRef get_return_address_fn_type(CodeGen *g) {
    ZigType *return_type = get_pointer_to_type(g, g->builtin_types.entry_u8, true);
    return LLVMFunctionType(get_llvm_type(g, return_type),
...
static LLVMValueRef get_return_address_fn_val(CodeGen *g) {
    ...
    LLVMTypeRef fn_type = get_return_address_fn_type(g);
...
static LLVMValueRef get_add_error_return_trace_addr_fn(CodeGen *g) {
    ...
    LLVMValueRef index_field_ptr = LLVMBuildStructGEP2(g->builder, arg_types[0], err_ret_trace_ptr, (unsigned)index_field_index, "");
    size_t addresses_field_index = g->stack_trace_type->data.structure.fields[1]->gen_index;
    LLVMValueRef addresses_field_ptr = LLVMBuildStructGEP2(g->builder, arg_types[1], err_ret_trace_ptr, (unsigned)addresses_field_index, "");

    ZigType *slice_type = g->stack_trace_type->data.structure.fields[1]->type_entry;
    size_t ptr_field_index = slice_type->data.structure.fields[slice_ptr_index]->gen_index;
    LLVMValueRef ptr_field_ptr = LLVMBuildStructGEP2(g->builder, arg_types[1], addresses_field_ptr, (unsigned)ptr_field_index, "");
    size_t len_field_index = slice_type->data.structure.fields[slice_len_index]->gen_index;
    LLVMValueRef len_field_ptr = LLVMBuildStructGEP2(g->builder, arg_types[1], addresses_field_ptr, (unsigned)len_field_index, "");
    ...
    LLVMValueRef address_slot = LLVMBuildInBoundsGEP2(g->builder, ptr_to_usize, ptr_value, address_indices, 1, "");
...
static LLVMValueRef get_return_err_fn(CodeGen *g) {
    ...
    LLVMValueRef return_address_ptr = LLVMBuildCall2(g->builder, get_return_address_fn_type(g), get_return_address_fn_val(g), &zero, 1, "");
...
static void add_sentinel_check(CodeGen *g, LLVMTypeRef sentinel_elem_ptr_ty, LLVMValueRef sentinel_elem_ptr, ZigValue *sentinel) {
    LLVMValueRef expected_sentinel = gen_const_val(g, sentinel, "");

    LLVMValueRef actual_sentinel = gen_load_untyped(g, sentinel_elem_ptr_ty, sentinel_elem_ptr, 0, false, "");
    LLVMValueRef ok_bit;
        ...
        ZigType *wanted_type, LLVMValueRef expr_val)
{
    ...
    return LLVMBuildCall2(g->builder, fn_type, func_ref, &expr_val, 1, "");
}
        ...
        LLVMValueRef val1, LLVMValueRef val2)
{
    ...
    LLVMValueRef result_struct = LLVMBuildCall2(g->builder, get_llvm_type(g, operand_type), fn_val, params, 2, "");
        ...
        if (abi_class == X64CABIClass_MEMORY || abi_class == X64CABIClass_MEMORY_nobyval) {
            ...
            switch (fn_walk->id) {
                ...
                case FnWalkIdCall: {
                    ...
                    LLVMValueRef loaded = LLVMBuildLoad2(g->builder, ptr_to_int_type_ref, bitcasted, "");
            ...
            switch (fn_walk->id) {
                ...
                case FnWalkIdCall: {
                    ...
                    for (uint32_t i = 0; i < number_of_regs; i += 1) {
                        ...
                        LLVMValueRef loaded = LLVMBuildLoad2(g->builder, abi_type, adjusted_ptr_to_struct, "");
                        fn_walk->data.call.gen_param_values->append(loaded);
                    }
                    break;
                }
...
// TODO specialized version for gep followed by a load
static LLVMValueRef build_struct_gep(CodeGen *g, ZigType *struct_type, LLVMValueRef val, int index) {
    ...
    assert(gen_index != SIZE_MAX);
    LLVMTypeRef field_type = struct_type->data.structure.fields[index]->type_entry->llvm_type;
    return LLVMBuildStructGEP2(g->builder, field_type, val, (unsigned)gen_index, "");
}
        ...
        Stage1AirInstSaveErrRetAddr *save_err_ret_addr_instruction)
{
    ...
    if (fn_is_async(g->cur_fn) && codegen_fn_has_err_ret_tracing_arg(g, ret_type)) {
        LLVMValueRef trace_ptr_ptr = LLVMBuildStructGEP2(g->builder, g->builtin_types.entry_any_frame->llvm_type,
...
static LLVMValueRef gen_resume(CodeGen *g, LLVMValueRef fn_val, LLVMValueRef target_frame_ptr, ResumeId resume_id) {
    LLVMTypeRef usize_type_ref = g->builtin_types.entry_usize->llvm_type;
    if (fn_val == nullptr) {
        LLVMValueRef fn_ptr_ptr = LLVMBuildStructGEP2(g->builder, g->builtin_types.entry_any_frame->llvm_type,
                target_frame_ptr, frame_fn_ptr_index, "");
        fn_val = LLVMBuildLoad2(g->builder, g->builtin_types.entry_any_frame->llvm_type, fn_ptr_ptr, "");
    }
...
static void gen_async_return(CodeGen *g, Stage1AirInstReturn *instruction) {
    ...
    LLVMValueRef prev_val = gen_maybe_atomic_op(g, LLVMAtomicRMWBinOpXor, g->builtin_types.entry_any_frame->llvm_type,
    ...
    if (ret_type_has_bits) {
        ...
        LLVMValueRef awaiter_ret_ptr_ptr = LLVMBuildStructGEP2(g->builder, g->builtin_types.entry_any_frame->llvm_type, g->cur_frame_ptr, frame_ret_start + 1, "");
        LLVMValueRef awaiter_ret_ptr = LLVMBuildLoad2(g->builder, g->builtin_types.entry_any_frame->llvm_type, awaiter_ret_ptr_ptr, "");
        ...
        if (codegen_fn_has_err_ret_tracing_arg(g, ret_type)) {
            LLVMValueRef awaiter_trace_ptr_ptr = LLVMBuildStructGEP2(g->builder, g->builtin_types.entry_any_frame->llvm_type, g->cur_frame_ptr,
                    frame_index_trace_arg(g, ret_type) + 1, "");
            LLVMValueRef dest_trace_ptr = LLVMBuildLoad2(g->builder, g->builtin_types.entry_any_frame->llvm_type, awaiter_trace_ptr_ptr, "");
            bool is_llvm_alloca;
...
static LLVMValueRef ir_render_return(CodeGen *g, Stage1Air *executable, Stage1AirInstReturn *instruction) {
    ...
    {
        if (instruction->operand == nullptr) {
            // TODO(gwenzek): check if we should use gen_load_untyped instead
            LLVMValueRef by_val_value = gen_load(g, g->cur_ret_ptr, fn_type_id->return_type, "");
            LLVMBuildRet(g->builder, by_val_value);
        } else {
            LLVMValueRef value = ir_llvm_value(g, instruction->operand);
            LLVMValueRef by_val_value = gen_load(g, value, fn_type_id->return_type, "");
            LLVMBuildRet(g->builder, by_val_value);
        }
...
static LLVMValueRef gen_float_op(CodeGen *g, LLVMValueRef val, ZigType *type_entry, BuiltinFnId op) {
    ...
    return LLVMBuildCall2(g->builder, type_entry->llvm_type, floor_fn, &val, 1, "");
}
    ...
    switch (op_id) {
        ...
        case IrBinOpSubWrap: {
            ...
            if (scalar_type->id == ZigTypeIdPointer) {
                ...
                // TODO runtime safety
                return LLVMBuildInBoundsGEP2(g->builder, op1->value->type->llvm_type, op1_value, &subscript_value, 1, "");
            } else if (scalar_type->id == ZigTypeIdFloat) {
                ZigLLVMSetFastMath(g->builder, ir_want_fast_math(g, &bin_op_instruction->base));
        ...
        Stage1AirInstPtrOfArrayToSlice *instruction)
{
    ...
    if (type_has_bits(g, actual_type)) {
        LLVMValueRef ptr_field_ptr = build_struct_gep(g, slice_type, result_loc, slice_ptr_index);
        ...
        LLVMValueRef slice_start_ptr = LLVMBuildInBoundsGEP2(g->builder, slice_ptr_type->llvm_type, expr_val, indices, 2, "");
    ...
    LLVMValueRef len_field_ptr = build_struct_gep(g, slice_type, result_loc, slice_len_index);
        ...
        Stage1AirInstLoadPtr *instruction)
{
    ...
    if (ptr_type->data.pointer.vector_index != VECTOR_INDEX_NONE) {
        LLVMValueRef index_val = LLVMConstInt(LLVMInt32Type(),
                ptr_type->data.pointer.vector_index, false);
        LLVMValueRef loaded_vector = LLVMBuildLoad2(g->builder, ptr_type->llvm_type, ptr, "");
    ...
    if (g->zig_target->arch == ZigLLVM_x86_64) {
            ...
            (g->zig_target->os == OsWindows && g->zig_target->abi != ZigLLVM_MSVC))
        {
            ...
            for (unsigned i = 0; i < 6; i += 1) {
                ...
                LLVMValueRef elem_ptr = LLVMBuildInBoundsGEP2(g->builder, array_type_ref, array_ptr, indexes, 2, "");
        ...
        (array_type->id == ZigTypeIdPointer && array_type->data.pointer.ptr_len == PtrLenSingle))
    {
            ...
            child_type->data.structure.layout == ContainerLayoutPacked)
        {
            ...
            if (host_int_bytes != 0) {
                ...
                LLVMValueRef elem_byte_ptr = LLVMBuildInBoundsGEP2(g->builder, ptr_u8_type_ref, u8_array_ptr, indices, 1, "");
        ...
        return LLVMBuildInBoundsGEP2(g->builder, array_ptr_type->llvm_type, array_ptr, indices, 2, "");
        ...
        return LLVMBuildInBoundsGEP2(g->builder, array_ptr_type->llvm_type, array_ptr, indices, 1, "");
        ...
        if (safety_check_on) {
            ...
            LLVMValueRef len_ptr = LLVMBuildStructGEP2(g->builder, array_ptr_type->llvm_type, array_ptr, (unsigned)len_index, "");
            LLVMValueRef len = gen_load(g, len_ptr, g->builtin_types.entry_usize, "");
        ...
        LLVMValueRef ptr_ptr = build_struct_gep(g, array_type, array_ptr, slice_ptr_index);
        ZigType *array_content_type = get_slice_pointee_type(array_type);
        LLVMValueRef ptr = gen_load(g, ptr_ptr, array_content_type, "");
        return LLVMBuildInBoundsGEP2(g->builder, array_content_type->llvm_type, ptr, &subscript_value, 1, "");
